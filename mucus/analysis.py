...
from rust_mucus import get_dist, get_dist_histogram
...
class Analysis:
    ...
    def rdf(self,
        ...
        #NOTE: this rdf using the rust get_dist_histogram function works.
        #      it gives the exact same results as the one using the old rdf function
        ...
        if self._exists(key_dict) and not overwrite:
            fname = get_path(self.cfg, filetype=key_dict["key"])
            with h5py.File(fname, "r", locking=self.h5pylock) as fh5:
                key = key_dict["key"].key
                r = fh5[key][0,:]
        ...
        if r_range is None:
                r_range = (0.0, float(self.cfg.lbox/2))
        else: 
            r_range = tuple(np.array(r_range).astype(float))
        ...
        # initialize rdf array
        g_r = np.zeros(n_bins)
        g_r_frame = np.zeros(n_bins).astype(int)
        ...
        with h5py.File(fname_h5, "r", locking=self.h5pylock) as fh5:
            for i, frame_idx in enumerate(self.frame_indices):
                ...
                get_dist_histogram(
                    fh5[Filetypes.Trajectory.key][frame_idx].astype(np.float64), 
                    g_r_frame, 
                    r_range,
                    n_bins,
                    self.cfg.lbox, 
    ...
    def virial_stress_tensor(self,
        ...
        # TODO check if this is correct
        #! PROBABLY WRONG SINCE I SHOULD DIVIDE BY RED2M^3
